...
export function seedJourneyBrowserScenario(scenario: PortableJourneyScenarioV1, storage: Pick<Storage, 'setItem'> = sessionStorage): void {
  writeJourneyBrowserConfig({ transportProfile: scenario.transportProfile, dnsProfile: scenario.dnsProfile, impairmentProfile: scenario.impairmentProfile }, storage);
}
