...
export function readJourneyBrowserConfig(storage: Pick<Storage, 'getItem'> = sessionStorage): JourneyScenarioConfig {
  ...
  return {
    transportProfile,
    dnsProfile,
    impairmentProfile: storedImpairment === 'dns-failure' || storedImpairment === 'server-failure' || storedImpairment === 'single-loss' || storedImpairment === 'latency-spike' || storedImpairment === 'route-failure' || storedImpairment === 'path-outage' || storedImpairment === 'congestion' ? storedImpairment : 'clean',
  };
}
