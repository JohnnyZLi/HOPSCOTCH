...
const BuilderEnterprisePanel = lazy(() => import('./BuilderEnterprisePanel.tsx'));
  ...
  return <section className={`builder-authoring-panel ${historical ? 'is-historical' : ''}`} aria-label="Builder authoring tools">
    ...
    <div className="builder-authoring-grid">
      ...
      <section className="builder-authoring-card"><div className="builder-authoring-card-title"><span>REUSE</span><strong>SITES + TEMPLATES</strong></div>
        ...
        {siteBounds.length>0&&<small>{siteBounds.length} SITE GROUP{siteBounds.length===1?'':'S'} PROJECTED IN THE MINIMAP · COLLAPSE CHANGES AUTHORING DETAIL ONLY, NEVER NETWORK STATE.</small>}
      </section>
    ...
    <section className="builder-authoring-card"><div className="builder-authoring-card-title"><span>ENTERPRISE</span><strong>L2 / L3 DEPTH</strong></div><div className="builder-authoring-toolbar compact"><button type="button" onClick={()=>setEnterpriseOpen((open)=>!open)}>{enterpriseOpen?'CLOSE ENTERPRISE':'OPEN ENTERPRISE'}</button></div>{enterpriseOpen&&<Suspense fallback={<small>LOADING ENTERPRISE TOOLS…</small>}><BuilderEnterprisePanel ethernet={snapshot.ethernet} historical={historical} onCommitEthernet={onCommitEthernet} onMessage={onMessage}/></Suspense>}</section>
