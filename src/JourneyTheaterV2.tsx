...
function TransportScene({ state }: { state: JourneyState }) {
  ...
  return <div className={`journey-scene transport-scene ${quic ? 'quic-transport-scene' : ''}`}>
...
function HttpScene({ state, hostname }: { state: JourneyState; hostname: string }) {
  ...
  const payloadLabel = state.http === 'request-sent' ? `GET / · ${hostname}` : state.http === 'headers' ? ':status 200' : state.http === 'streaming' || state.http === 'complete' ? 'RESPONSE DATA' : h3 ? 'CONTROL + QPACK' : 'SETTINGS';
  return <div className={`journey-scene http-scene ${h3 ? 'h3-scene' : ''}`}><div className="http-request-line"><span>{protocol}</span><strong>{payloadLabel}</strong></div><div className="http-stream"><span>{h3 ? 'REQUEST STREAM' : 'STREAM 1'}</span><i><b style={{width:`${progress}%`}}/></i><strong>{progress}%</strong></div><div className="http-stream muted"><span>TRANSPORT</span><i><b style={{width:`${Math.max(12,progress-14)}%`}}/></i><strong>{h3 ? 'QUIC 1-RTT' : 'TLS OVER TCP'}</strong></div><p>{h3 ? 'HTTP/3 maps request/response data to QUIC streams; there is no TCP connection beneath it.' : 'Application frames remain encrypted on the wire and inherit TCP delivery behavior.'}</p></div>;
}
...
function PacketScene({ state }: { state: JourneyState }) {
  ...
  return <div className={`journey-scene packet-scene ${quic ? 'quic-packet-scene' : ''}`}><div className="packet-layers">{layers.map(([label,value],index)=><div key={label} className={index===3?'encrypted':''}><span>{label}</span><strong>{value}</strong></div>)}</div><div className="packet-bytes">{['45','00','01','9A','00','01','40','00','40',quic?'11':'06','B7','5C','C0','00','02','0A','CB','00','71','2A'].map((byte,index)=><b key={`${byte}-${index}`} className={state.packet === 'headers' && index < 12 ? 'hot' : ''}>{byte}</b>)}</div><p>{quic ? 'UDP and QUIC headers remain structurally visible while the QUIC protected payload stays opaque without key material.' : state.packet === 'headers' ? 'Header bytes map to delivery fields while the TLS payload remains opaque.' : 'One representative frame is frozen without creating a second transfer.'}</p></div>;
}
...
export function JourneyTheater({ hostname, timeMs, startPlaying, evidence, onHostnameChange, onTimeChange, onEvidenceChange, onOpenDetail, onExit }: {
  ...
  const chooseProfile = (next: JourneyTransportProfile) => {
