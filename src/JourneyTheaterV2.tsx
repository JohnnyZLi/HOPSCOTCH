...
export function JourneyTheater({ hostname, timeMs, startPlaying, evidence, measuredState, onHostnameChange, onTimeChange, onEvidenceChange, onOpenDetail, onExit }: {
  ...
  return <VisualWorkspaceShell
    ...
    toolbar={<><div className="visual-identity"><i/><span>Request journey</span><strong>{hostname} · {profileLabel} · {dnsLabel}</strong></div><div className="journey-visual-tools"><VisualDrawerTabs active={activeDrawer} items={[{id:'inspect',label:'Inspect'},{id:'config',label:'Configure'},{id:'events',label:'Events',badge:String(scenario.events.length)},{id:'evidence',label:'Evidence',badge:evidence?'On':undefined}]} onSelect={openDrawer}/><button type="button" className="visual-tool-button" onClick={onExit}>Exit</button></div></>}
    ...
    timeline={<VisualTimeRail timeMs={timeMs} durationMs={scenario.durationMs} playing={playing} playbackSpeed={playbackSpeed} onPlaybackSpeedChange={setPlaybackSpeed} label="Request journey" context={`${profileLabel} · ${dnsLabel} · ${godModeLabel}`} events={timelineEvents} milestones={timelineMilestones} onToggle={togglePlayback} onReset={()=>seek(0)} onSeek={seek}/>}
  >
