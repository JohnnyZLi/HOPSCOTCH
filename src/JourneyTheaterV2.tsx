  ...
  return <motion.section className="journey-workspace" data-profile={profile} data-dns-profile={dnsProfile} data-impairment={impairmentProfile} data-modifiers={selectedModifiers.join(' ')} initial={reduceMotion ? {opacity:1}:{opacity:0,scale:.985}} animate={{opacity:1,scale:1}} exit={{opacity:0}}>
    ...
    <div className="journey-main">
      <section className="journey-stage">
        ...
        <div className="journey-state-strip"><div><span>DNS</span><strong className={dnsFailureSelected?toneClass:''}>{dnsStateLabel}</strong></div><div><span>ROUTE</span><strong className={routeSelected?toneClass:''}>{state.route.toUpperCase()}</strong></div><div><span>{profile==='quic-h3'?'QUIC':'TCP'}</span><strong className={lossSelected||latencySelected||outageSelected||congestionSelected?toneClass:''}>{transportStateLabel}</strong></div><div><span>TLS</span><strong>{state.tls.toUpperCase()}</strong></div><div><span>{profile==='quic-h3'?'H3':'H2'}</span><strong>{state.http.toUpperCase()}</strong></div><div><span>PACKET</span><strong>{state.packet.toUpperCase()}</strong></div></div>
      </section>

      <aside className="journey-rail"><section className="journey-context"><div className="rail-title"><span>ENDPOINT CONTEXT</span><strong>{evidence?'ATTACHED':'SIMULATION ONLY'}</strong></div>{evidence?<><div className="context-facts"><div><b>EDGE OBSERVED</b><strong>{evidence.edge.asn?`AS${evidence.edge.asn}`:'ASN UNAVAILABLE'}</strong><small>{evidence.edge.colo??'COLO UNAVAILABLE'}</small></div><div><b>PUBLIC COLLECTOR</b><strong>{evidence.routing.originAsns.length?evidence.routing.originAsns.map((asn)=>`AS${asn}`).join(' / '):'ORIGIN UNAVAILABLE'}</strong><small>{evidence.routing.prefix??'PREFIX UNAVAILABLE'}</small></div></div><p><b>DECORATION ONLY.</b> These observations do not choose {profileLabel}, {dnsLabel}, or {godModeLabel} and do not become the simulated path.</p></>:<p>Attach optional Cloudflare/RIPE context. The selected {profileLabel} · {dnsLabel} · {godModeLabel} story remains deterministic.</p>}</section><section className="journey-events"><div className="rail-title"><span>CAUSAL CHAIN</span><strong>{String(state.activeEventIndex+1).padStart(2,'0')} / {scenario.events.length}</strong></div><div className="journey-event-list" ref={eventRailRef}>{scenario.events.map((current,index)=>{const complete=current.atMs<=timeMs;const active=current.id===state.activeEvent.id;const lossEvent=isLossEvent(current.kind);const latencyEvent=isLatencyEvent(current.kind);const routeEvent=isRouteFailureEvent(current.kind);const congestionEvent=isCongestionEvent(current.kind);const dnsFailureEvent=isDnsFailureEvent(current.kind);const serverFailureEvent=isServerFailureEvent(current.kind);return <button type="button" key={current.id} className={`journey-event ${complete?'complete':''} ${active?'current':''} ${lossEvent?'impairment-event':''} ${latencyEvent?'latency-event':''} ${routeEvent?'route-event':''} ${congestionEvent?'congestion-event':''} ${dnsFailureEvent?'dns-failure-event':''} ${serverFailureEvent?'server-failure-event':''}`} onClick={()=>seek(current.atMs)}><span>{String(index+1).padStart(2,'0')}</span><div><strong>{current.title}</strong><small>{formatTime(current.atMs)} · {current.scale.toUpperCase()} · {current.protocol}</small></div><i className={provenanceClass(current.provenance)}/></button>})}</div></section></aside>
    </div>

    <footer className="journey-time-machine"><div className="journey-time-controls"><button type="button" onClick={togglePlayback}>{playing?'Ⅱ':'▶'}</button><button type="button" onClick={()=>seek(0)}>↺</button></div><div className="journey-time-readout"><span>GLOBAL TIME MACHINE · {profileLabel} · {dnsLabel} · {godModeLabel}</span><strong>{formatTime(timeMs)}</strong></div><div className="journey-scrubber"><div>{scenario.events.map((current)=><i key={current.id} className={`${current.atMs<=timeMs?'passed':''} ${isLossEvent(current.kind)&&current.kind!=='transport.recovered'?'impairment-marker':''} ${isLatencyEvent(current.kind)?'latency-marker':''} ${isRouteFailureEvent(current.kind)?'route-marker':''} ${isCongestionEvent(current.kind)?'congestion-marker':''} ${isDnsFailureEvent(current.kind)?'dns-failure-marker':''} ${isServerFailureEvent(current.kind)?'server-failure-marker':''}`} style={{left:`${current.atMs/scenario.durationMs*100}%`}}/>)}</div><input type="range" min="0" max={scenario.durationMs} step="10" value={Math.round(timeMs)} onChange={(event)=>seek(Number(event.currentTarget.value))}/></div><span className="journey-duration">{formatTime(scenario.durationMs)}</span></footer>
  </motion.section>;
}