...
function routeWhy(graph:BuilderGraph,routing:BuilderRoutingConfig,entry:BuilderRouteTableEntry):BuilderWorkbenchWhyStep[]{
  ...
  if(entry.source==='bgp'){
    const bgp=builderBgpState(graph,{segments:{},defaultGateways:{}} as BuilderAddressing,routing.bgp);
...
function routedStateSections(input:BuilderDeviceWorkbenchInput,deviceId:string):BuilderWorkbenchSection[]{
  ...
  if(node.kind==='router'){
    routeTableForBuilderRouter(input.graph,input.addressing,input.routing,deviceId).forEach((entry)=>routeRows.push(row(`state:route4:${entry.id}`,`IPV4 ${entry.source.toUpperCase()}`,entry.prefix,`${entry.nextHop?`via ${entry.nextHop}`:'DIRECT'} · ${entry.outgoingInterface} · AD ${entry.administrativeDistance} · M ${entry.metric} · ${entry.stateNote}`,entry.active?'good':'bad',routeWhy(input.graph,input.routing,entry))));
