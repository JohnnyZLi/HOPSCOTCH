...
import { cloneBuilderIpv6Config, createDefaultBuilderIpv6Config, reconcileBuilderIpv6Config, traceBuilderIpv6Forwarding, type BuilderIpv6Config } from './builder/ipv6.ts';
...
export function NetworkBuilder({ onExit, onOpenFailureStory, onOpenProbePacket, initialGraph = defaultBuilderGraph, initialLayout = defaultBuilderLayout, initialAddressing, initialRouting, initialEthernet, initialNat, initialIpv6, initialSourceId = 'client', initialDestinationId = 'app', stressLabel }: { onExit: () => void; onOpenFailureStory: () => void; onOpenProbePacket?: (seed: BuilderProbePacketSeed) => void; initialGraph?: BuilderGraph; initialLayout?: BuilderLayout; initialAddressing?: BuilderAddressing; initialRouting?: BuilderRoutingConfig; initialEthernet?: BuilderEthernetConfig; initialNat?: BuilderNatConfig; initialIpv6?: BuilderIpv6Config; initialSourceId?: string; initialDestinationId?: string; stressLabel?: string }) {
  ...
  const ipv6ForwardingTrace = useMemo(() => traceBuilderIpv6Forwarding(graph, ipv6, sourceId, destinationId), [graph, ipv6, sourceId, destinationId]);
  ...
  const ipv6ForwardingLinks = new Set(ipv6ForwardingTrace.hops.flatMap((hop) => hop.linkId ? [hop.linkId] : []));
  ...
  const saveScenario = () => {
    try {
      ...
      const scenario = createBuilderScenario(scenarioName.trim() || 'Untitled topology', graph, sourceId, destinationId, layout, addressing, routing, existing, ethernet, linkProfiles, acl, nat, undefined, ipv6);
  ...
  const exportScenario = () => {
    try {
      const scenario = createBuilderScenario(scenarioName.trim() || 'Exported topology', graph, sourceId, destinationId, layout, addressing, routing, undefined, ethernet, linkProfiles, acl, nat, undefined, ipv6);
      ...
      setMessage('Scenario v9 exported with dual-stack routed topology, link characteristics, ACL/NAT policy, and Ethernet/STP configuration; ARP/NAT-session/probe/FDB observations remain session-only.');
    ...
    <motion.section className="builder-workspace" data-stress-label={stressLabel} data-node-count={graph.nodes.length} data-link-count={graph.links.length} initial={reduceMotion ? { opacity: 1 } : { opacity: 0, scale: .985 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
      ...
      <div className="builder-main">
        ...
        <aside className="builder-controls">
          ...
          {!stressLabel&&<BuilderIpv6Panel graph={graph} ipv4={addressing} ipv6={ipv6} selectedNodeId={selectedNodeId} selectedLinkId={selectedLinkId} sourceId={sourceId} destinationId={destinationId} onChange={setIpv6} onMessage={setMessage}/>}
