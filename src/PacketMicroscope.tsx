...
import { SimulatedPacketMechanism } from './SimulatedPacketMechanism.tsx';
import {
  VisualDrawerTabs,
...
import './SimulatedPacketMicroscopePass.css';
import {
  buildPacket,
  defaultPacketConfig,
  hex16,
...
function SimulatedPacketMicroscope({
  ...
  return (
    ...
    >
      <VisualWorkspaceShell
        className="packet-visual-workspace interactive-world-workspace simulated-packet-workspace"
      ...
      >
      <div className="packet-stage simulated-packet-stage">
        <section className="simulated-packet-mechanism-stage" aria-label="Recomputed packet assembly">
          <motion.div className="simulated-packet-annotation simulated-packet-annotation--source" initial={reduceMotion ? false : { opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: reduceMotion ? 0 : .5, delay: reduceMotion ? 0 : .12 }}>
            <span>SCENARIO SOURCE</span>
            <strong>{origin?.label ?? 'FAILURE SEQUENCE · TRAFFIC RECOVERS'}</strong>
          </motion.div>
          <motion.div className="simulated-packet-annotation simulated-packet-annotation--time" initial={reduceMotion ? false : { opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: reduceMotion ? 0 : .5, delay: reduceMotion ? 0 : .18 }}>
            <span>SIMULATION TIME</span>
            <strong>{origin?.timestamp ?? '00:05.400'}</strong>
          </motion.div>
          <SimulatedPacketMechanism
            snapshot={snapshot}
            selectedLayer={selectedLayer}
            selectedField={selectedField}
            onSelectLayer={(segment) => {
              setSelectedLayer(segment.id);
              setSelectedFieldId(segment.fields[0]?.id ?? null);
            }}
          />
          <div className="simulated-packet-annotation simulated-packet-annotation--route">
            <span>MODELED ROUTE</span>
            <strong>{networkSegment.label} · {transportSegment.label} · {sourceAddress} → {destinationAddress}</strong>
          </div>
          <div className="simulated-packet-annotation simulated-packet-annotation--boundary">
            <span>TRUTH BOUNDARY</span>
            <strong>GENERATED BY MODEL · FCS NOT MODELED</strong>
          </div>
        </section>

        <section className="simulated-byte-workbench" aria-label="Generated byte and field workbench">
          <div className="packet-hex-panel">
            <div className="packet-panel-heading">
              <div><span>GENERATED FRAME BYTES</span><strong>SELECT A FIELD TO TRACE THE MODEL INTO EXACT OFFSETS</strong></div>
              <span>{snapshot.bytes.length} bytes</span>
            </div>
            <div className="packet-hex-grid" aria-label="Generated packet bytes">
              {snapshot.bytes.map((byte, index) => {
                ...
                return (
                  <span
                    key={`${index}-${byte}`}
                    className={`packet-byte byte-${segment.id}${highlighted ? ' highlighted' : ''}`}
                    title={`frame byte ${index}`}
                  >
                    {hexByte(byte)}
                  </span>
                );
          ...
          <article className="packet-field-lens simulated-field-lens">
            ...
            <p>{selectedField ? `${selectedField.value} · ${selectedField.length > 0 ? `FRAME BYTES ${selectedSegment.offset + selectedField.offset}–${selectedSegment.offset + selectedField.offset + selectedField.length - 1}` : 'DERIVED VALUE'}` : 'Open Inspect to map protocol structure to exact generated bytes.'}</p>
