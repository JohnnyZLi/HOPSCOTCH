...
import BuilderWorkbenchDepthPanel from './BuilderWorkbenchDepthPanel.tsx';
import type { BuilderWorkbenchInspection } from './BuilderDeviceWorkbench.tsx';
...
export default function BuilderDeviceWorkbenchContent({snapshot,options,onSelect,onInspect,historicalSequence=null,diff=null}:{snapshot:BuilderDeviceWorkbenchSnapshot;options:BuilderDeviceOption[];onSelect:(ref:BuilderDeviceRef)=>void;onInspect?:(inspection:BuilderWorkbenchInspection)=>void;historicalSequence?:number|null;diff?:BuilderTimelineDeviceDiff|null;}){
  ...
  const inspect=(nextTab:WorkbenchTab,device:BuilderDeviceRef=snapshot.device)=>onInspect?.({tab:nextTab,device});
  return <section className={`builder-device-workbench ${historical?'is-historical':''}`} data-device-plane={snapshot.device.plane} data-device-id={snapshot.device.id} data-history-sequence={historicalSequence??'live'}>
    ...
    <label>DEVICE<select value={keyFor(snapshot.device)} onChange={(event)=>{const [plane,id]=event.currentTarget.value.split(':',2);const ref={plane:plane as BuilderDeviceRef['plane'],id};onSelect(ref);inspect(tab,ref);}}>{['ROUTED GRAPH','ETHERNET FABRIC'].map((group)=><optgroup key={group} label={group}>{options.filter((option)=>option.group===group).map((option)=><option key={keyFor(option)} value={keyFor(option)}>{option.label} · {option.kind}</option>)}</optgroup>)}</select></label>
