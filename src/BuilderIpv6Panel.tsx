...
export function BuilderIpv6Panel({ graph, ipv4, ipv6, selectedNodeId, selectedLinkId, sourceId, destinationId, controlState, onControlStateChange, lifecycleState, onLifecycleStateChange, routingDepth, onRoutingDepthChange, probePacketBytes, onProbePacketBytesChange, onChange, onMessage }: {
  ...
  return <section className="builder-ipv6-section">
    ...
    {selectedNode?.kind==='router'?<><div className="button-row"><button type="button" disabled={!ipv6.enabled} onClick={toggleOspfv3}>{selectedOspfv3Enabled?'DISABLE OSPFV3':'ENABLE OSPFV3'}</button><button type="button" disabled={!ipv6.enabled} onClick={()=>{onChange(setBuilderOspfv3Everywhere(graph,ipv4,ipv6,true));onMessage('OSPFV3 AREA 0 ENABLED · all routers advertise connected IPv6 /64s over link-local adjacencies.');}}>ENABLE ALL OSPFV3</button><button type="button" onClick={()=>{onChange(setBuilderOspfv3Everywhere(graph,ipv4,ipv6,false));onMessage('OSPFV3 DISABLED · O6 routes withdrawn; C6/S6 remain.');}}>DISABLE ALL OSPFV3</button></div><div className="builder-ospf-neighbors">{ospfv3.adjacencies.filter((entry)=>entry.aRouterId===selectedNode.id||entry.bRouterId===selectedNode.id).length===0?<small>NO OSPFV3 ROUTER NEIGHBORS</small>:ospfv3.adjacencies.filter((entry)=>entry.aRouterId===selectedNode.id||entry.bRouterId===selectedNode.id).map((entry)=>{const neighbor=entry.aRouterId===selectedNode.id?entry.bRouterId:entry.aRouterId;return <div key={entry.id} className={entry.state==='FULL'?'full':'down'}><span>{entry.state}</span><strong>{labelFor(graph,neighbor)}</strong><small>{entry.linkId.toUpperCase()} · AREA {routingDepth.linkAreas[entry.linkId] ?? 0} · COST {entry.cost} · {entry.reason}</small></div>;})}</div></>:<small className="builder-routing-note">Select a router to inspect OSPFv3 adjacency and enablement.</small>}
