...
function TransactionPacketMicroscope({ packet, transaction, onClose }: { packet: BuilderApplicationPacket; transaction: BuilderApplicationTransaction; onClose: () => void }) {
  return <div className="builder-app-microscope"><PacketMicroscope onExit={onClose} initialConfig={packet.config} origin={{ label: `APPLICATION TRANSACTION · ${packet.label}`, timestamp: transaction.id, actionLabel: 'RETURN TO APPLICATION TRANSACTION ↗' }} /></div>;
}
  ...
  return (
    <section className="builder-app-panel" data-track-d="shared-application-transaction">
      <header className="builder-app-heading">
        <div><span>APPLICATION TRANSACTION</span><strong>ONE REQUEST · ONE CAUSAL TRUTH STACK</strong><p>Addressing/DHCP → Ethernet/VLAN/STP → ARP/ND → FIB → ACL/NAT → link truth → canonical transport/TLS/application → exact packet bytes.</p></div>
      ...
      {!transaction ? <div className="builder-app-empty"><strong>NO APPLICATION TRANSACTION YET</strong><p>Choose a hosted service and run the request. Transport and application stages do not exist until lower-layer truth passes.</p></div> : <>
        ...
        {diagnosis&&<div className={`builder-app-diagnosis ${diagnosis.firstBrokenDimension?'failed':'passed'}`}><span>CAUSAL DIAGNOSIS</span><strong>{diagnosis.summary}</strong><p>{diagnosis.dimensions.map((entry)=>`${entry.id} ${entry.status}`).join(' · ')}</p><small>FIRST BROKEN BOUNDARY IS DERIVED FROM THE SHARED APPLICATION TRANSACTION · NOT_REACHED NEVER COUNTS AS FAILURE.</small></div>}
