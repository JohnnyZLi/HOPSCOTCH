...
export default function BuilderChallengePanel({
  ...
  const devices = challenge.broken.graph.nodes.map((node) => ({ id: node.id, label: node.label, kind: node.kind }));
  ...
  return <section className={`builder-challenge-panel ${score.solved ? 'is-solved' : ''}`} data-challenge-id={challenge.id} data-challenge-score={score.total}>
    ...
    <div className="builder-challenge-instructions">
      ...
      <p>Run the ordinary Builder Ping / Traceroute tools and inspect CONFIG / STATE / EVENTS in Device Workbench. Repair the network with the normal Builder configuration controls, then prove the repair with another probe.</p>
    </div>

    <div className="builder-challenge-hypothesis">
      <span>CAUSAL HYPOTHESIS</span>
      {hypothesis
        ? <strong>{hypothesis.boundary} · {devices.find((device) => device.id === hypothesis.deviceId)?.label ?? hypothesis.deviceId}</strong>
        : <>
          ...
          <label>RESPONSIBLE DEVICE<select value={deviceId} disabled={historical} onChange={(event) => setDeviceId(event.currentTarget.value)}><option value="">CHOOSE…</option>{devices.map((device) => <option key={device.id} value={device.id}>{device.label} · {device.kind.toUpperCase()}</option>)}</select></label>
      ...
      <small>Reasoning points require both probe evidence and device inspection before the hypothesis can score.</small>
    </div>
    ...
    {score.solved && <div className="builder-challenge-solved">
      ...
      <strong>{challenge.fault.boundary} · {devices.find((device) => device.id === challenge.fault.nodeId)?.label ?? challenge.fault.nodeId}</strong>
      <p>The endpoint had no default gateway. Restoring canonical gateway <b>{challenge.fault.expectedGateway}</b> repaired forwarding, and your post-repair probe verified the outcome.</p>
    </div>}
