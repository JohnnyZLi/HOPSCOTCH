...
if anchor not in text: raise SystemExit('EVPN assertion anchor missing')
...
if old not in text: raise SystemExit('doc BGP boundary anchor missing')
text=text.replace(old,new,1).replace("- EVPN Type-2 and Type-3 learning,","- EVPN Type-2 and Type-3 learning plus iBGP split-horizon non-propagation without a route reflector,",1)
d.write_text(text)

Path('scripts/track-g-bgp-semantics.py').unlink()
Path('scripts/track-g-trigger.txt').unlink(missing_ok=True)
