...
assert.equal(migratedV2.version, 6);
...
assert.equal(deserializeBuilderScenario(JSON.stringify(legacyV1)).version, 6);
...
assert.equal(migratedV3.version, 6);
...
assert.equal(migratedV4.version, 6);
...
console.log(`Builder L3 addressing contract passed: ${graph.links.length} deterministic segments, endpoint gateways, topology reconciliation, schema-v6 round trip, and v1–v4 migration without changing route truth.`);
