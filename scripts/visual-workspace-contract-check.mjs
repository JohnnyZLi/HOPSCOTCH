...
assert.match(asTheater, /SIMULATED BEST PATH/);
...
assert.match(capturedPacket, /Packet evidence/);
assert.match(capturedPacket, /<CapturedFrameMechanism/);
assert.match(capturedPacket, /mode="microscope"/);
assert.match(capturedPacket, /captured-byte-workbench/);
assert.ok(!capturedPacket.includes('className="packet-origin-strip"'), 'captured Packet Microscope must keep origin as a peripheral annotation');
assert.ok(!capturedPacket.includes('className="packet-object-wrap"'), 'captured Packet Microscope must not restore generic packet slabs');
assert.ok(!capturedPacket.includes('className="packet-relations"'), 'captured Packet Microscope must not restore the duplicate metadata dashboard band');
assert.match(capturedPacketCss, /\.captured-microscope-mechanism-stage/);
assert.match(capturedPacketCss, /grid-template-columns: minmax\(0, 1fr\) clamp\(235px, 25vw, 340px\)/);
