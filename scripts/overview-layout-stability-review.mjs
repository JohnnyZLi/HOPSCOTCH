...
const outputDir = resolve(root, 'artifacts/overview-layout-stability-review');
...
async function launchChrome(chromePath) {
  ...
  const userDataDir = mkdtempSync(join(tmpdir(), 'hopscotch-overview-stability-'));
...
async function measureViewport(cdp, origin, width, height) {
  ...
  await screenshot(cdp, `overview-${width}x${height}.png`);
  return metrics;
}
...
async function main() {
  ...
  if (report.failures.length > 0) throw new Error(`Overview layout stability review failed:\n${report.failures.join('\n')}`);
