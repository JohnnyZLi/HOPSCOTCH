...
for(const id of ['r1','r2','r3'])rrBgp=setBuilderBgpRouterAsn(rrGraph,rrBgp,id,64512);
...
const bgpAddressing=createDefaultBuilderAddressing(bgpGraph);let bgp=createDefaultBuilderBgpConfig();bgp=setBuilderBgpRouterAsn(bgpGraph,bgp,'a',64520);bgp=setBuilderBgpRouterAsn(bgpGraph,bgp,'b',64521);bgp=upsertBuilderBgpSession(bgpGraph,bgp,'a-b','peer');const ab=bgp.sessions.find((entry)=>entry.linkId==='a-b');assert.ok(ab);
bgp=upsertBuilderBgpOrigin(bgpGraph,bgp,{routerId:'a',prefix:'198.51.100.0/24',med:0,communities:['64520:100'],description:'community policy'});
bgp=upsertBuilderBgpPolicy(bgpGraph,bgp,{routerId:'a',direction:'export',sessionId:ab.id,order:10,action:'permit',prefix:'198.51.100.0/24',setLocalPref:null,setMed:null,addCommunity:'64520:200',matchCommunity:'64520:100',removeCommunity:'64520:100',prependAsCount:2,allowRelationshipLeak:false,description:'community + prepend'});
let bgpState=builderBgpState(bgpGraph,bgpAddressing,bgp);const bRoute=bgpState.bestRoutes.find((route)=>route.routerId==='b'&&route.prefix==='198.51.100.0/24');assert.ok(bRoute);assert.ok(bRoute.communities.includes('64520:200'));assert.equal(bRoute.communities.includes('64520:100'),false);assert.ok(bRoute.asPath.filter((asn)=>asn===64520).length>=3,'export AS prepend must add deterministic sender-AS copies before normal eBGP prepend');
